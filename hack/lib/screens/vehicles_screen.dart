  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await _showVehicleDialog(context);
          _load();
        },
      ...
      trailing: Row(
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () async {
              await _showVehicleDialog(context, vehicle: v);
              _load();
            },
          ),
          IconButton(
  ...
  Future<void> _confirmDelete(Vehicle v) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Voulez-vous supprimer le véhicule ${v.plateNumber} ?',
    ...
    if (ok == true && v.id != null) {
      await DbHelper.instance.deleteVehicle(v.id!);
      _load();
    }
  }

  Future<void> _showVehicleDialog(BuildContext ctx, {Vehicle? vehicle}) async {
    final isEdit = vehicle != null;
    final plateCtrl = TextEditingController(text: vehicle?.plateNumber);
    final ownerCtrl = TextEditingController(text: vehicle?.ownerName);
    final notesCtrl = TextEditingController(text: vehicle?.notes);
    VehicleCategory selected = vehicle?.category ?? VehicleCategory.visitor;
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (isEdit) {
                  await DbHelper.instance.updateVehicle(v);
                } else {
                  await DbHelper.instance.upsertVehicle(v);
                }
